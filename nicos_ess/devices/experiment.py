...
from nicos.core import SIMULATION, Override, Param, UsageError, \
    relative_path, listof, mailaddress
...
class EssExperiment(Experiment):
    parameters = {
        'cache_filepath':
            Param('Path to the proposal cache',
                  ...
                  userparam=False),
        'filewriter_base':
            Param('Relative data path on the file writer server under which all '
    ...
    def proposalpath_of(self, proposal):
        return path.join(self.filewriter_base, time.strftime('%Y'), proposal)
