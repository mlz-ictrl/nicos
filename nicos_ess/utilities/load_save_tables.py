...
def load_table_from_csv(table, headers, filename):
    ...
    -----------
        ...
        filename (str): Path to the filename
        headers (List[str], optional): List of column names to fill.
            Defaults to None. If None, only the headers in file will be filled
    ...
    with open(filename, "r") as file:
        ...
        headers_read = next(reader)

        # If headers were modified by hand in the file by user
        if not set(headers_read).issubset(set(headers)):
        ...
        indices = [i for i, e in enumerate(headers) if e in headers_read]
        ncols = len(headers)
