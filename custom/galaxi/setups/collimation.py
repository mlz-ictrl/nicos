# -*- coding: utf-8 -*-

description = 'GALAXI motors'

group = 'optional'

tango_base = 'tango://localhost:10000/galaxi/'

devices = dict(
    b1_h     = device('nicos.devices.tango.Motor',
                      description = 'Collimation slit B1 Height',
                      tangodevice = tango_base + 'fzjs7/B1H',
                      lowlevel = True,
                     ),
    b1h      = device('nicos.devices.generic.Axis',
                      description = 'Collimation slit B1 Height',
                      motor = 'b1_h',
                      coder = 'b1_h',
                      precision = 0.01,
                      maxtries = 1,
                     ),
    b1_b     = device('nicos.devices.tango.Motor',
                      description = 'Collimation slit B1 Width',
                      tangodevice = tango_base + 'fzjs7/B1B',
                      lowlevel = True,
                     ),
    b1b      = device('nicos.devices.generic.Axis',
                      description = 'Collimation slit B1 Width',
                      motor = 'b1_b',
                      coder = 'b1_b',
                      precision = 0.01,
                      maxtries = 1,
                     ),
    b1       = device('nicos.devices.generic.TwoAxisSlit',
                      description = 'Slit b1 width and hight',
                      pollinterval = 10,
                      maxage = 61,
                      fmtstr = '%.2f %.2f',
                      horizontal = 'b1b',
                      vertical = 'b1h',
                     ),
    b1_y     = device('nicos.devices.tango.Motor',
                      description = 'Collimation slit B1 Left Right',
                      tangodevice = tango_base + 'fzjs7/B1Y',
                      lowlevel = True,
                     ),
    b1y      = device('nicos.devices.generic.Axis',
                      description = 'Collimation slit B1 Left Right',
                      motor = 'b1_y',
                      coder = 'b1_y',
                      precision = 0.01,
                      maxtries = 1,
                     ),
    b1_z     = device('nicos.devices.tango.Motor',
                      description = 'Collimation slit B1 Up Down',
                      tangodevice = tango_base + 'fzjs7/B1Z',
                      lowlevel = True,
                     ),
    b1z      = device('nicos.devices.generic.Axis',
                      description = 'Collimation slit B1 Up Down',
                      motor = 'b1_z',
                      coder = 'b1_z',
                      precision = 0.01,
                      maxtries = 1,
                     ),
    b2_h     = device('nicos.devices.tango.Motor',
                      description = 'Collimation slit B2 Height',
                      tangodevice = tango_base + 'fzjs7/B2H',
                      lowlevel = True,
                     ),
    b2h      = device('nicos.devices.generic.Axis',
                      description = 'Collimation slit B2 Height',
                      motor = 'b2_h',
                      coder = 'b2_h',
                      precision = 0.01,
                      maxtries = 1,
                     ),
    b2_b     = device('nicos.devices.tango.Motor',
                      description = 'Collimation slit B2 Width',
                      tangodevice = tango_base + 'fzjs7/B2B',
                      lowlevel = True,
                     ),
    b2b      = device('nicos.devices.generic.Axis',
                      description = 'Collimation slit B2 Width',
                      motor = 'b2_b',
                      coder = 'b2_b',
                      precision = 0.01,
                      maxtries = 1,
                     ),
    b2       = device('nicos.devices.generic.TwoAxisSlit',
                      description = 'Slit b2 width and hight',
                      pollinterval = 10,
                      maxage = 61,
                      fmtstr = '%.2f %.2f',
                      horizontal = 'b2b',
                      vertical = 'b2h',
                     ),
    b2_y     = device('nicos.devices.tango.Motor',
                      description = 'Collimation slit B2 Left Right',
                      tangodevice = tango_base + 'fzjs7/B2Y',
                      lowlevel = True,
                     ),
    b2y      = device('nicos.devices.generic.Axis',
                      description = 'Collimation slit B2 Left Right',
                      motor = 'b2_y',
                      coder = 'b2_y',
                      precision = 0.01,
                      maxtries = 1,
                     ),
    b2_z     = device('nicos.devices.tango.Motor',
                      description = 'Collimation slit B2 Up Down',
                      tangodevice = tango_base + 'fzjs7/B2Z',
                      lowlevel = True,
                     ),
    b2z      = device('nicos.devices.generic.Axis',
                      description = 'Collimation slit B2 Up Down',
                      motor = 'b2_z',
                      coder = 'b2_z',
                      precision = 0.01,
                      maxtries = 1,
                     ),
    b3_h     = device('nicos.devices.tango.Motor',
                      description = 'Collimation slit B3 Height',
                      tangodevice = tango_base + 'fzjs7/B3H',
                      lowlevel = True,
                     ),
    b3h      = device('nicos.devices.generic.Axis',
                      description = 'Collimation slit B3 Height',
                      motor = 'b3_h',
                      coder = 'b3_h',
                      precision = 0.01,
                      maxtries = 1,
                     ),
    b3_b     = device('nicos.devices.tango.Motor',
                      description = 'Collimation slit B3 Width',
                      tangodevice = tango_base + 'fzjs7/B3B',
                      lowlevel = True,
                     ),
    b3b      = device('nicos.devices.generic.Axis',
                      description = 'Collimation slit B3 Width',
                      motor = 'b3_b',
                      coder = 'b3_b',
                      precision = 0.01,
                      maxtries = 1,
                     ),
    b3       = device('nicos.devices.generic.TwoAxisSlit',
                      description = 'Slit b3 width and hight',
                      pollinterval = 10,
                      maxage = 61,
                      fmtstr = '%.2f %.2f',
                      horizontal = 'b3b',
                      vertical = 'b3h',
                     ),
    b3_y     = device('nicos.devices.tango.Motor',
                      description = 'Collimation slit B3 Left Right',
                      tangodevice = tango_base + 'fzjs7/B3Y',
                      lowlevel = True,
                     ),
    b3y      = device('nicos.devices.generic.Axis',
                      description = 'Collimation slit B3 Left Right',
                      motor = 'b3_y',
                      coder = 'b3_y',
                      precision = 0.01,
                      maxtries = 1,
                     ),
    b3_z     = device('nicos.devices.tango.Motor',
                      description = 'Collimation slit B3 Up Down',
                      tangodevice = tango_base + 'fzjs7/B3Z',
                      lowlevel = True,
                     ),
    b3z      = device('nicos.devices.generic.Axis',
                      description = 'Collimation slit B3 Up Down',
                      motor = 'b3_z',
                      coder = 'b3_z',
                      precision = 0.01,
                      maxtries = 1,
                     ),
)
