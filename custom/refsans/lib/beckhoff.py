...
class BeckhoffMotor(TacoDevice, HasTimeout, Motor):
    """
    ...
    def doInit(self, mode):
        ...
        if not (0x3000 <= self.address <= 0x47ff) or \
            ((self.address & 0xfff) - 0x20) % 10:
    ...
    def _speed2phys(self, speed):
        # see manual
        return speed  / float(self.slope * 1.6384e-2)

    def _phys2speed(self, value):
        # see manual
        return int(value * self.slope * 1.6384e-2)
...
class BeckhoffMotorAllParams(BeckhoffMotor):
    """
    Device object for a digital output device via a Beckhoff modbus interface.
