...
devices = {
    ...
    '%s_watertemp' % setupname: device('devices.tango.Sensor',
    ...
    '%s_waterflow' % setupname: device('devices.tango.Sensor',
    ...
    '%s_heater' % setupname: device('devices.tango.NamedDigitalOutput',
                               description = 'On/Off-State of the heater',
                               tangodevice = tango_base + 'plc/_heater',
                               mapping = {'on': 1, 'off': 0}
                              ),
    '%s_pumpstate' % setupname: device('devices.tango.NamedDigitalInput',
                               description = 'On/Off-State of the turbo pump',
                               tangodevice = tango_base + 'plc/_state',
                               mapping = {'on': 1, 'off': 0}
                              ),
    '%s_vacuum' % setupname: device('devices.tango.Sensor',
