description = 'virtual CARESS HWB Devices'
...
#                 ),
    transm = device('nicos.devices.vendor.caress.Motor',
                    description = 'virtual HWB TRANSM',
                    fmtstr = '%.2f',
                    unit = 'mm',
                    abslimits = (-360, 360),
                    userlimits = (-200, 200),
    ...
    mon = device('nicos.devices.vendor.caress.Counter',
                 description = 'virtual HWB MON',
                 fmtstr = '%d',
                 nameserver = '%s' % (nameservice,),
                 objname = '%s' % (servername),
                 config = 'MON 116 11 0x1000000 1',
                ),
    tim1 = device('nicos.devices.vendor.caress.Timer',
                  description = 'virtual HWB TIM1',
                  fmtstr = '%.2f',
                  unit = 's',
                  nameserver = '%s' % (nameservice,),
                  objname = '%s' % (servername),
    ...
    tim2 = device('nicos.devices.vendor.caress.Timer',
                  description = 'virtual HWB TRANSM',
                  fmtstr = '%.2f',
                  unit = 's',
                  nameserver = '%s' % (nameservice,),
    ...
    adet = device('nicos.devices.vendor.caress.MultiChannelDetector',
