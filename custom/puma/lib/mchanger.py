...
from nicos.core import PositionError, Moveable, Readable, Param, ProgrammingError, \
...
class Mchanger(Moveable):
    ...
    attached_devices = {
        ...
        'depot'         : (Moveable, 'Depot'),
        'r3'            : (Moveable, '3R coupling'),
        'lift'          : (Moveable, 'Lift'),
        'grip'          : (Moveable, 'Gripper'),
        'mlock'         : (Moveable, 'Magnetic lock'),
        'holdstat'      : (Readable, 'Read status of monochromators holders of'
                                     ' the depot'),
        'foch'          : (Moveable, 'Horizontal focusing'),
        'focv'          : (Moveable, 'Vertical focusing'),
#       'mono_stat'     : (Readable, 'Read status of monochromators on the'
#                                    ' monotable'),
    }
...
#                            self.name)
    ...
    def _startCheck(self):
        ...
        if lift_pos != 'ref':
            raise PositionError(self, 'Lift is not at reference position.'
                     'Please check if mono is fixed at the depot or at the monotable')
        return carpos
    ...
    def _move2start(self):
        ...
        for dev, pos in self._changing_positions.items():
            dev.start(pos)

        for dev in self._changing_positions:
            dev.wait()
    ...
    def _moveUp(self,pos):
        ...
        #~ self.log.info('Moving depot to %r' % pos)
        #~ self._adevs['depot'].start(pos)
        #~ self.log.info('Open Grip.')
        #~ self._adevs['grip'].start('open')
        #~ self.log.info('Moving Lift to bottom position.')
        #~ self._adevs['lift'].start('bott')
        #~ self.log.info('Close Grip.')
        #~ self._adevs['grip'].start('closed')
        #~ self.log.info('Open 3R coupling.')
        #~ self._adevs['r3'].start('open')
        #~ self.log.info('Open Maglock.')
        #~ self._adevs['mlock'].start('open')
        #~ self.log.info('Moving Lift to top2 position.')
        #~ self._adevs['lift'].start('top2')
        #~ self.log.info('Close Maglock.')
        #~ self._adevs['mlock'].start('closed')
        #~ self.log.info('Moving Lift to top1 position.')
        #~ self._adevs['lift'].start('top1')
        #~ self.log.info('Open Grip.')
        #~ self._adevs['grip'].start('open')
        #~ self.log.info('Moving Lift to reference position.')
        #~ self._adevs['lift'].start('ref')
        #~ self.log.info('Close Grip.')
        #~ self._adevs['grip'].start('closed')
    ...
    def _moveDown(self,pos):
        ...
        #~ self.log.info('Moving Lift to bottom position.')
        #~ self._adevs['lift'].start('bott')
        #~ self.log.info('Close 3R coupling.')
        #~ self._adevs['r3'].start('closed')
        #~ self.log.info('Open Grip.')
        #~ self._adevs['grip'].start('open')
        #~ self.log.info('Moving Lift to reference position.')
        #~ self._adevs['lift'].start('ref')
