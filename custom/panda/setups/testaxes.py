...
devices = dict(
    ...
    #~ bus = device('nicos.vendor.ipc.IPCModBusTCP',
                 #~ #loglevel = 'debug',
                 #~ loglevel = 'info',
		 #~ roundtime = 0.05,
                 #~ host = '172.25.15.51'),
        # using serial cable
    #bus = device('nicos.vendor.ipc.IPCModBusSerial',
    #~ #           loglevel = 'debug',
    #~ #           host = '/dev/ttyS0'),
    ...
    bus3 = device('nicos.vendor.ipc.IPCModBusTaco',
            tacodevice='//pandasrv/panda/moxa/port3',
            loglevel='info',
            timeout=0.5, ),
            
    bus4 = device('nicos.vendor.ipc.IPCModBusTaco',
            tacodevice='//pandasrv/panda/moxa/port4',
            loglevel='info',
            timeout=0.5, ),
            
    bus5 = device('nicos.vendor.ipc.IPCModBusTaco',
...
for i in range(1,9):
    devices['s%d' % i] = device('nicos.vendor.ipc.Motor', bus='bus2', addr=0x50+i, slope=1., unit='steps', abslimits=(0, 999999),fmtstr='%d')
    devices['p%d' % i] = device('nicos.vendor.ipc.Coder', bus='bus2', addr=0x60+i, slope=1., unit='steps')
    devices['c%d' % i] = device('nicos.vendor.ipc.Coder', bus='bus2', addr=0x70+i, slope=1., unit='steps')
