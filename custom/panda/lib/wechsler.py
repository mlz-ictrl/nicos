...
class Beckhoff(Device):
    """ Qick and Dirty implementation of a communication class with Beckhoff
    device over ModbusTCP
    """
    parameters = {
        'host': Param('Hostname or IP-Adress of Beckhoff device',
                      type=str, default='wechsler.panda.frm2', settable=True),
        'addr': Param('ModBus unit address', type=int, default=0,
                      settable=True),
    }
        ...
        while True:
            try:
                ...
                while True:
                    ...
                    while sent < len(request):
                        ...
                        try:
                            ...
                            self.log.debug('Communicate: sending failed, '
                                           'closing down')
                            sent = 0
                            break
                    ...
                    # data=''
                    # while len(data)<7:
                    # just some number bigger than 260
                    #     data+=sock.recv(500)
                    ...
                    try:
                        # transaction ID, protocol ID, numbytes, subunit
                        d = unpack('>HHHB', data)
                    except Exception:
                        self.log.debug('Communicate: unpacking failed !')
                        break
                    self.log.debug('Communicate: reading %d more bytes' %
                                   (d[2]-1))
                    data += sock.recv(d[2]-1)
                    self.log.debug('Communicate: read %d more bytes, %r' %
                                   (d[2]-1, data))
                    # s=''
                    # for i in request:
                    #     s = s+':%02x' % ord(i)
                    ...
                    #     s = s+':%02x' % ord(i)
                    # print s[1:]
                    # print 'request of %d Bytes gave response of %d Bytes' % \
                    #       (len(request),len(data))
                    # dt('result')
                    self.log.debug('Communicate: return result and await next '
                                   'request')
                    # return response and request new request
                    request = yield(data[7:])
                ...
                try:
                    ...
                    pass
                try:
                    sock.close()  # be kind and close down everything
                except Exception:
                    pass
                self.log.debug('Communicate: socket closed, looping outer '
                               'loop again')
            except Exception as err:
                self.log.warning('Communicate: Something bad happened, '
                                 'relooping: %s' % err)
                request = yield ('')
        self.log.debug('Communicate: This should never happen: End of While '
                       'True: Loop !!!')
    ...
    def ReadBitOutput(self, addr):
        request = pack('>HHHBBHH', random.getrandbits(16), 0, 6, self.addr, 1,
                       addr, 1)  # can read more than 1 bit !
    ...
    def ReadBitInput(self, addr):
        request = pack('>HHHBBHH', random.getrandbits(16), 0, 6, self.addr, 2,
                       addr, 1)  # can read more than 1 bit !
        response = self.communicate(request)
        FOE, _, status = unpack('>BBB', response)
        if FOE == 2:
    ...
    def ReadWordOutput(self, addr):
        ...
        request = pack('>HHHBBHH', random.getrandbits(16), 0, 6, self.addr, 3,
                       addr, 1)  # can read more than 1 word !
        response = self.communicate(request)
    ...
    def ReadWordInput(self, addr):
        request = pack('>HHHBBHH', random.getrandbits(16), 0, 6, self.addr, 4,
                       addr, 1)  # can read more than 1 word !
        response = self.communicate(request)
        FOE, _, status = unpack('>BBH', response)
        if FOE == 4:
    ...
    def WriteBitOutput(self, addr, value):
        ...
        request = pack('>HHHBBHH', random.getrandbits(16), 0, 6, self.addr, 5,
                       addr, value)  # can write exactly 1 word !
        response = self.communicate(request)
    ...
    def WriteWordOutput(self, addr, value):
        ...
        request = pack('>HHHBBHH', random.getrandbits(16), 0, 6, self.addr, 6,
                       addr, value)  # can write exactly 1 word !
        response = self.communicate(request)
        try:
            FOE, _, status = unpack('>BHH', response)
    ...
    def ReadBitsOutput(self, addr, num):
        if num > 2000:
            raise ValueError('%d Bits are too much for ReadBitsOutput!' % num)
        request = pack('>HHHBBHH', random.getrandbits(16), 0, 6, self.addr, 1,
                       addr, num)
        response = self.communicate(request)
        m = (num+7)/8
        ...
        FOE = data[0]
        if FOE != 1:
            raise Exception('Returned function should be 1 but is 0x%02x!' %
                            FOE)
        return [(data[i/8 + 2] >> (i & 7)) & 0x01 for i in range(num)]

    def ReadBitsInput(self, addr, num):
        if num > 2000:
            raise ValueError('%d Bits are too much for ReadBitsInput!' % num)
        request = pack('>HHHBBHH', random.getrandbits(16), 0, 6, self.addr, 2,
                       addr, num)
        response = self.communicate(request)
        m = (num+7)/8
        ...
        FOE = data[0]
        if FOE != 2:
            raise Exception('Returned function should be 2 but is 0x%02x!' %
                            FOE)
        return [(data[i/8 + 2] >> (i & 7)) & 0x01 for i in range(num)]
    ...
    def ReadWordsOutput(self, addr, num):
        if addr < 0x800:
            addr = addr | 0x800  # beckhoff special....
        if num > 125:
            raise ValueError('%d Words are too much for ReadWordsOutput!' %
                             num)
        request = pack('>HHHBBHH', random.getrandbits(16), 0, 6, self.addr, 3,
                       addr, num)  # can read more than 1 word !
        response = self.communicate(request)
    ...
    def ReadWordsInput(self, addr, num):
        if num > 125:
            raise ValueError('%d Words are too much for ReadWordsInput!' % num)
        request = pack('>HHHBBHH', random.getrandbits(16), 0, 6, self.addr,
                       4, addr, num)  # can read more than 1 word !
        response = self.communicate(request)
        data = unpack('>BB%dH' % num, response)
        FOE = data[0]
        if FOE == 4:
    ...
    def WriteBitsOutput(self, addr, values):
        ...
        data.append(b)
        m = len(data)
        request = pack('>HHHBBHHB%dB' % m, random.getrandbits(16), 0, 7+m,
                       self.addr, 15, addr, len(values), m, *data)
        response = self.communicate(request)
    ...
    def WriteWordsOutput(self, addr, values):
        ...
        m = len(values)
        request = pack('>HHHBBH%dH' % m, random.getrandbits(16), 0, 4+2*m,
                       self.addr, 16, addr, *values)
        response = self.communicate(request)
        try:
    ...
    # ''' Beckhoff registers consist of a pair of two adresse: at baseaddr is
    # the index+r/Wflag, at baseaddr+1 is the data
...
class MonoWechsler(Device):
    ...
    def Lift_obere_Ablage(self):
        ...
        if r == '10':
            return False
        raise HWError('Taster Lift obere Ablageposition (fast ganz oben)'
                      'defekt')
    ...
    def MagazinID(self):
        try:
            r = (self.MagazinID1() and '1' or '0') + \
                (self.MagazinID2() and '1' or '0') + \
                (self.MagazinID3() and '1' or '0')
        except Exception:
            r = (self.MagazinID1() and '1' or '0') + \
                (self.MagazinID2() and '1' or '0') + \
                (self.MagazinID3() and '1' or '0')
    ...
    def Lift_Bremse(self, *args):
        if len(args) == 0:  # read_access
            ...
            if r[0] == '1':
                raise HWError('Bremse Lift Drahtbruch !')
            if r[1] == '1':
    ...
    def Magazin_Bremse(self, *args):
        if len(args) == 0:  # read_access
            ...
            if r[0] == '1':
                raise HWError('Bremse Magazin Drahtbruch !')
            if r[1] == '1':
    ...
    def InhibitRelay(self, *args):
        if len(args) == 0:
            ...
            if r[0] == '1':
                raise HWError('InhibitRelay Drahtbruch !')
            if r[1] == '1':
    ...
    def FahreLiftMotor(self, speed):
        ''' Schaltet LiftMotor an, speed>0 -> hoch, speed<0 runter,
        speed=0 schaltet alles ab
    ...
    def FahreMagazinMotor(self, speed):
        ''' Schaltet MagazinMotor an, speed>0 -> CW, speed<0 CCW,
        speed=0 schaltet alles ab
    ...
    def Fahre(self, Motor, Goal, speed=3000, timeout=None):
        ...
        while not(mygoal(Goal)):  # not there yet ?
            if timeout and timeoutime < time():  # time is up?
                Motor(0)  # STOP
                raise HWError('Motor %s didn\'t reach Goal %s within %d '
                              'seconds' % (Motor.__name__,
                                           Goal.__name__, timeout))
    ...
    def PrepareChange(self):
        ...
        if self.NotAus():
            raise UsageError(self, 'NotAus (Emergency stop) activated, please '
                             'check and retry!')
        ...
        if not(self.Magazin_Klammer_geschlossen()) or \
           self.Magazin_Klammer_offen():
            raise HWError(self, 'Magazinklammer nicht geschlossen !')
        if (not(self.Liftgreifer_links_geschlossen()) or
           not(self.Liftgreifer_rechts_geschlossen()) or
           self.Liftgreifer_links_offen() or self.Liftgreifer_rechts_offen()):
            raise HWError(self, 'Liftgreifer nicht geschlossen !')
        if self.Lift_untere_Ablage() or self.Lift_obere_Ablage() or \
           self.Lift_Absetzposition():
        ...
        #     maw(mfh,0)
        #     mfh.power = 0  # go to known good value and switch off
        #     maw(mfv,0)
        #     mfv.power = 0
        # except Exception:
        #     pass
        ...
        #     raise HWError(self, 'Enable-Signal form Motorrahmen missing, '
        #                   'check cabling !')
    ...
    def MagazinSelect(self, pos):
        # faehrt angeforderte Magazinposition an den Lift
        # (pos in self.positions)
        currentpos = self.positions.index(self.MagazinID())
        mpos = self.positions.index(pos)  # make positions numeric.....
        if mpos > currentpos:
            self.Fahre(self.FahreMagazinMotor,
                       lambda: not(self.Magazin_Referenzposition()), 3000, 6)
            self.Fahre(self.FahreMagazinMotor, self.Magazin_Referenzposition,
                       10000, 30)
            self.Fahre(self.FahreMagazinMotor,
                       lambda: not(self.Magazin_Referenzposition()), 3000, 6)
            self.Fahre(self.FahreMagazinMotor, self.Magazin_Referenzposition,
    ...
    def LiftSelect(self, pos):
        # faehrt angeforderte Liftposition an
        # 0=unten
        # 1=park
        # 2=ablageu
        # 3=ablageo
        currentpos = -1
        if self.Lift_Absetzposition():
            currentpos = 0
        elif self.Lift_Parkposition():
            currentpos = 1
        elif self.Lift_untere_Ablage():
            currentpos = 2
        elif self.Lift_obere_Ablage():
            currentpos = 3
        if currentpos == -1:
            raise HWError('Lifposition not determined !!!!!')
        if pos > currentpos:
            self.Fahre(self.FahreLiftMotor, [self.Lift_Absetzposition,
                                             self.Lift_Parkposition,
                                             self.Lift_untere_Ablage,
                                             self.Lift_obere_Ablage][pos],
                       30000, 300)
        elif pos < currentpos:
            self.Fahre(self.FahreLiftMotor, [self.Lift_Absetzposition,
                                             self.Lift_Parkposition,
    ...
    def close_Liftgreifer(self):
        ...
        for _ in range(50):
            if self.Liftgreifer_geschlossen():
                return
            sleep(0.1)
    ...
    def close_MagazinKlammer(self):
        ...
        for _ in range(50):
            if self.Magazin_Klammer_geschlossen():
                return
            sleep(0.1)
    ...
    def CheckMagazinSlotEmpty(self, slot):
        ...
        if slot == '111' and (self.Monogreifer_111_Links() or
                              self.Monogreifer_111_Rechts()):
            raise HWError(self, 'Magazin for Position 111 is already occupied,'
                          ' please check!')
        if slot == '110' and (self.Monogreifer_110_Links() or
                              self.Monogreifer_110_Rechts()):
            raise HWError(self, 'Magazin for Position 110 is already occupied,'
                          ' please check!')
        if slot == '101' and (self.Monogreifer_101_Links() or
                              self.Monogreifer_101_Rechts()):
            raise HWError(self, 'Magazin for Position 101 is already occupied,'
                          ' please check!')
        if slot == '011' and (self.Monogreifer_011_Links() or
                              self.Monogreifer_011_Rechts()):
            raise HWError(self, 'Magazin for Position 011 is already occupied,'
                          ' please check!')

    def CheckMagazinSlotUsed(self, slot):
        # checks if the given slot in the magazin is used (contains a
        # monoframe)
        if not(slot in self.positions):
            raise UsageError(self, 'Got Illegal MagazinIDCode to check')
        if slot == '111' and not(self.Monogreifer_111_Links() and
                                 self.Monogreifer_111_Rechts()):
            raise HWError(self, 'Magazin for Position 111 is empty, '
                          'please check!')
        if slot == '110' and not(self.Monogreifer_110_Links() and
                                 self.Monogreifer_110_Rechts()):
            raise HWError(self, 'Magazin for Position 110 is empty, '
                          'please check!')
        if slot == '101' and not(self.Monogreifer_101_Links() and
                                 self.Monogreifer_101_Rechts()):
            raise HWError(self, 'Magazin for Position 101 is empty, '
                          'please check!')
        if slot == '011' and not(self.Monogreifer_011_Links() and
                                 self.Monogreifer_011_Rechts()):
            raise HWError(self, 'Magazin for Position 011 is empty, '
                          'please check!')
    ...
    def Transfer_Mono_Magazin2Lift(self):
        ...
        # Magazin should not contain a mono
    ...
    def Transfer_Mono_Lift2Magazin(self):
        ...
        for _ in range(3):
            ...
            self.close_MagazinKlammer()
            self.LiftSelect(2)
        # XXX maybe we should check here that the mono is indeed in the
        # magazin, or it might drop!!!
    ...
    def Transfer_Mono_Lift2Monotisch(self):
        # XXX TODO: Absetzparameter fuer Monos ermitteln und position
        # anfahren (eigentlich vorher!)
        # ab hier nur das reine absetzen aus dem Lift heraus, der auf
        # Parkposition stehen sollte
        if not(self.Lift_Parkposition()):
        ...
        self.LiftSelect(1)
        self.close_Liftgreifer()
        # XXX TODO: activiate Mono specific stuff (mfv/mfh.....) and switch
        # Monodeviceswitcher
    ...
    def change(self, old, whereto):
        ...
        if not(old in self.monos + ['None']):
            raise UsageError(self, '\'%s\' is illegal value for Mono, use one '
                             'of' % old + ', '.join(self.monos))
        if not(whereto in self.monos + ['None']):
            raise UsageError(self, '\'%s\' is illegal value for Mono, use one '
                             'of' % whereto + ', '.join(self.monos))
        self.PrepareChange()
        ...
        if whereto != 'None':
            ...
            # if it's in the magazin it's not on the table
            self.CheckMagazinSlotUsed(monocode_new)
        if self.mono_on_table != 'None':
            monocode_old = self.positions[self.monos.index(self.mono_on_table)]
            # if it's on the table, it isnt in the magazin
            self.CheckMagazinSlotEmpty(monocode_old)

        # Ok, so first thing is to move the mono (if any) from the table to the
        # magazin
        if self.mono_on_table != 'None':
            self.LiftSelect(1)    # Make sure, we are at the parking position
            # rotate magazin to the position according to the old mono
            self.MagazinSelect(monocode_old)
            self.Transfer_Mono_Monotisch2Lift()
            self.Transfer_Mono_Lift2Magazin()
            self.mono_on_table = 'None'  # no longer a Mono on the table
        # go to parking position to allow rotation of magazin
        self.LiftSelect(1)
    ...
    def printstatusinfo(self):
        ...
        self.log.info('Lift is at ' +
                      ['Absetzposition', 'Parkposition', 'untere Ablage',
                       'obere Ablage', 'unknown position'][currentpos])
        self.log.info('Liftgreifer should be ' + (self.Lift_Druckluft() and
                                                  'open' or 'closed') +
                      ' and are ' + (self.Liftgreifer_offen() and 'open' or
                                     (self.Liftgreifer_geschlossen() and
                                      'closed' or 'undetermined')))
        try:
            self.log.info('Magazin is at ' + self.MagazinID() + ' which is '
                          'assigned to ' + self.monos[
                              self.positions.index(self.MagazinID())])
        except Exception:
            self.log.error('Magazin is at unknown position')
        self.log.info('Magazin_Klammer should be ' + (
            self.Magazin_Druckluft() and 'open' or 'closed') + ' and are ' +
            (self.Magazin_Klammer_offen() and 'open' or (
                self.Magazin_Klammer_geschlossen() and 'closed' or
                'undetermined')))
        self.log.info('MagazinSlot 111 is ' + (
            (self.Monogreifer_111_Links() and 'occupied' or
             (self.Monogreifer_111_Rechts() and 'unknown' or 'free')))
            + ' (%s)' % self.monos[self.positions.index('111')])
        self.log.info('MagazinSlot 011 is ' + (
            (self.Monogreifer_011_Links() and 'occupied' or
             (self.Monogreifer_011_Rechts() and 'unknown' or 'free')))
            + ' (%s)' % self.monos[self.positions.index('011')])
        self.log.info('MagazinSlot 101 is ' + (
            (self.Monogreifer_101_Links() and 'occupied' or
             (self.Monogreifer_101_Rechts() and 'unknown' or 'free')))
            + ' (%s)' % self.monos[self.positions.index('101')])
        self.log.info('MagazinSlot 110 is ' + (
            (self.Monogreifer_110_Links() and 'occupied' or
             (self.Monogreifer_110_Rechts() and 'unknown' or 'free')))
            + ' (%s)' % self.monos[self.positions.index('110')])
        self.log.info('MonoKupplung is ' + (self.Mono_Druckluft() and 'open'
                                            or 'closed'))

    # dangerous stuff to aid in recovering
    # @usercommand
    def init_Liftpos(self):
        if self.Lift_Absetzposition() or self.Lift_Parkposition() or \
           self.Lift_untere_Ablage() or self.Lift_obere_Ablage():
            return
        # no luck, scan lift downwards!
        # THIS IS DANGEROUS IF THE LIFT EVER HAPPEN TO GO BELOW THE LOWEST
        # SWITCH !!!
        # this might take a while....
        self.Fahre(self.FahreLiftMotor,
                   lambda: self.Lift_Absetzposition() or
                   self.Lift_Parkposition() or self.Lift_untere_Ablage()
                   or self.Lift_obere_Ablage(), -5000, 600)
    ...
    def init_Liftpos_hoch(self):
        ...
        # THIS IS DANGEROUS IF THE LIFT EVER HAPPEN TO GO ABOVE THE HIGHEST
        # SWITCH !!!
        # this might take a while....
        self.Fahre(self.FahreLiftMotor,
                   lambda: self.Lift_Absetzposition() or
    ...
    def init_Magazinpos_cw(self):
        ...
        # THIS IS DANGEROUS IF THE LIFT EVER HAPPEN TO GO BELOW THE LOWEST
        # SWITCH !!!
        self.Fahre(self.FahreMagazinMotor, self.Magazin_Referenzposition, 500,
                   60)  # this might take a while....
