...
class Beckhoff(TacoDevice, Device):
    ...
    def ReadWord(self, addr):
        addr += self.baseaddr
        if self.cache_baseaddr <= addr <= self.cache_baseaddr + self.cache_numregs:
            return self._cached_registers[addr - self.cache_baseaddr]
        return self._taco_guard(self._dev.readHoldingRegisters, (0, addr, 1))[0]
    ...
    def ReadDWord(self, addr):
        addr += self.baseaddr
        if self.cache_baseaddr <= addr <= self.cache_baseaddr + self.cache_numregs - 1:
    ...
    def run(self):
        if self.value is not None:
            if getattr(self.obj, self.key) != self.value:
                raise HWError(self, '%s.%s should be %r !' %(self.obj, self.key, self.value))
...
class Changer(BaseSequencer):
    attached_devices = {
        'lift'         : Attach('Lift moving a mono up or down between 4 positions',
                                Moveable),
        'magazine'     : Attach('Magazine holding the monos and having 4 positions',
                                Moveable),
    ...
    parameters = {
        ...
        'mono_in_lift': Param('Which mono is in the lift',
                          type=oneof('PG','Si','Cu','Heusler','None'),
                          default='None', settable=True, userparam=False),
        'exchangepos' : Param('dict of device names to positional values for changing monos',
    ...
    def PrepareChange(self) :
        ...
        # XXX TODO: store old values of positions and offsets someplace to recover after changing back
    ...
    def _start(self, seq):
        if self._seq_thread is not None:
            raise MoveError(self, 'Can not start sequence, device is still busy')
    ...
    def change(self, old, whereto):
        ...
        if self.mono_on_table != old:
            raise UsageError(self, 'Mono %s is not supposed to be on the table, '
                                   '%s is!'%(old,self.mono_on_table))

        seq = []
        # 1) move away the old mono, if any
        if old != 'None':
            seq.extend(self._gen_table2lift())
            seq.extend(self._gen_lift2mag(self.positions[self.monos.index(old)]))
    ...
    def printstatusinfo(self):
        self.log.info('PLC is %s' % ('enabled' if self._adevs['enable'].read() == 0xef14 else 'disabled'))
        self.log.info('Inhibit_realy is %s' % self._adevs['inhibitrelay'].read())
        liftposnames = {1:'Absetzposition', 2:'Parkposition', 3:'untere Ablage', 4:'obere Ablage'}
        self.log.info('lift is at %s' % liftposnames[self._adevs['lift'].read()])
        try:
