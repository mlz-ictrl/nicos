...
class Coder(NicmCoder):
    ...
    def doInit(self):
        try :
	    self._dev = TACOCoder(self.getTacodevice())
        except TACOClient.TACOError, e:
            self.printerror()
            raise CommunicationError("TACO Coder device '%s' not available\nTACO error code: %d ; %s " % (self.getTacodevice(), e.errcode, e), 0)
        except Exception, e:
            self.printerror()
            raise e
        ...
        try:
            ...
            try:
                ...
                raise CommunicationError("TACOError: %d; Cannot switch on Coder device : '%s'\n%s " % (e.errcode, self._dev.deviceName(), e), 0)
            except Exception, e:
                self.printerror()
