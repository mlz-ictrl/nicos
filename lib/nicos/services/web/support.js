...
function Console() {
    ...
    res = $.ajax({url: '/json', dataType: 'json',
                  data: '{"method": "start_session", "params": []}',
...
Console.prototype.getOutput = function() {
    ...
    $.ajax({url: '/json', dataType: 'json',
            data: '{"method": "output", "params": ["' + this.sid + '"]}',
            type: 'POST', timeout: 1000000, success: function(data) {
                if (data.result) {
                    self.addOutput(data);
                    self.getOutput();
                } else {
                ...
                if (status == 'timeout') {
                    ...
                    self.addError({error: 'Error requesting new output.'});
                }
            }
           });
}

Console.prototype.clickHandler = function(ev) {
    $('#et-' + ev.target.id.substr(3)).toggle();
}

Console.prototype.addOutput = function(data) {
    var html = $(data.result);
    html.filter('.exc-line').click(this.clickHandler);
    html.insertBefore(this.cursor);
    window.scrollBy(0, document.documentElement.scrollHeight);
}

Console.prototype.addError = function(data) {
    $('<div>').text(data.error).addClass('error').insertBefore(this.cursor);
    $('<div>').text('Please reload the page to reactivate the session.').
        addClass('error').insertBefore(this.cursor);
}
