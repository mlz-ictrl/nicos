...
sysconfig = dict(datasinks = ['jbi_liveview', 'FileWriterControl', 'FileWriterControlDr'],)

includes = ['polariser', 'chopper_sim']
...
devices = dict(
    ...
    det = device('nicos_sinq.amor.devices.detector.PolarizationSwitchDetector',
                 ...
                 liveinterval = 5.0,
                 counters = ['roi1'],
                 postprocess = [('roi1', 'hist_yz')],
                 visibility = ('metadata', 'namespace'),
                 polarization = 'polarization',
                 filewritercontrol = 'FileWriterControl',
                 images = ['hist_tofz', 'hist_yz'],
                 ),
    ...
    FileWriterControl = device('nicos_sinq.amor.devices.file_writer.AmorFileWriterControlSink',
    ...
    FileWriterStatusDr = device('nicos_sinq.devices.datasinks.file_writer.FileWriterStatus',
                                description = 'Status of the file-writer',
                                brokers = configdata('config.KAFKA_BROKERS'),
                                statustopic = configdata('config.FILEWRITER_DR_STATUS_TOPIC'),
                                unit = '',
                                ),
    FileWriterControlDr = device('nicos_sinq.amor.devices.file_writer.AmorDrFileWriterControlSink',
                                 description = 'Control for the file-writer',
                                 brokers = configdata('config.KAFKA_BROKERS'),
                                 pool_topic = configdata('config.FILEWRITER_DR_POOL_TOPIC'),
                                 status = 'FileWriterStatusDr',
                                 nexus = 'NexusStructure',
                                 filenametemplate = ['amor%(year)sn%(pointcounter)06d.hdf'],
                                 visibility = ('metadata', 'namespace'),
                                 one_file_per_scan = False,
