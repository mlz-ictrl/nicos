...
class JustBinItImage(ImageChannelMixin, PassiveChannel):
    parameters = {
        ...
        'response_topic': Param('Response to just-bin-it commands',
                               type=str, userparam=False, settable=False,
                               mandatory=False,
                               ),
        ...
        'no_message_timeout': Param('How long to wait for a message before switching to an error state',
                                    type=intrange(1,20), default=5, userparam=True, settable=True, unit='s',
                          ),
        # Keeping this in the cache, one hopes that Nicos could be reset,
        # without the histogramming being interrupted, of course the datasinks
    ...
    def _consume(self, consumer):
        try:
            return consumer.consume(num_messages=1, timeout=0.5)
        except RuntimeError:
            self._status = status.ERROR, 'Histogram consumer is closed'
        except (KafkaError, KafkaException):
            self._status = status.ERROR, 'Internal Kafka error'
        except ValueError:
            self._status = status.ERROR, 'Num messages > 1M'
        return []

    def _get_new_messages(self):
        try:
            while True:
                self._process_or_raise(self._consume(self._histograms_consumer))
                if self.response_topic:
                    self._process_or_raise(self._consume(self._response_consumer))
        except Exception as error:
            self._status = status.ERROR, f"Kafka Consumer Died with Exception {type(error).__name__}: {error}"
    ...
    def _process_or_raise(self, messages):
        for msg in messages:
            ...
            try:
                ...
                if contents.get("response", '') == "ERR":
                    self._status = status.ERROR, contents.get("message", "Unknown Error")
                ...
                try:
                    ...
                    self._status = status.ERROR, "Unable to deserialise message"
                    ...
                    if info['state'] == 'ERROR':
                        ...
                        self._status = status.ERROR, error_msg
                        continue
        ...
        if self._status[0] == status.BUSY and (time.time() - self._last_msg_time) > self.no_message_timeout:
            self._status = status.ERROR, f'No messages since {datetime.fromtimestamp(self._last_msg_time)}'
