description = 'Sample position 2 motorization'
group = 'lowlevel'
display_order = 30

pvprefix = 'SQ:NEUTRA:turboPmacSample:'
devices = dict(
    sp2_tx = device('nicos_sinq.devices.epics.motor.SinqMotor',
        description = 'Sample Position 2, Translation X',
        motorpv = pvprefix + 'sp2_tx',
    ),
    sp2_ty = device('nicos_sinq.devices.epics.motor.SinqMotor',
        description = 'Sample Position 2, Translation Y',
        motorpv = pvprefix + 'sp2_ty',
    ),
    sp2_tz = device('nicos_sinq.devices.epics.motor.SinqMotor',
        description = 'Sample Position 2, Translation Z',
        motorpv = pvprefix + 'sp2_tz',
    ),
    sp2_mm = device('nicos_sinq.devices.epics.dynamic.MasterMacsNode',
                    pvprefix = 'NEUTRA:MMSP2',
                    pvsuffixes = configdata('config.dynamic_motor_suffixes'),
                    visibility = {'metadata', 'namespace'},
    )
)
