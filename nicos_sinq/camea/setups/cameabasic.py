...
devices = dict(
    ...
    s2t = device('nicos_sinq.devices.epics.sinqmotor_deprecated.SinqMotor',
        description = 'Sample two theta',
        motorpv = pvmcu1 + '2t',
    ),
    m2t = device('nicos_sinq.devices.epics.sinqmotor_deprecated.SinqMotor',
    ...
    tl_sim = device('nicos.devices.generic.manual.ManualMove',
        description = 'Simulated tl motor',
        abslimits = (-15, 15),
        default = 0,
        unit = 'mm',
    ),
    tu_sim = device('nicos.devices.generic.manual.ManualMove',
        description = 'Simulated tu motor',
        abslimits = (-15, 15),
        default = 0,
        unit = 'mm',
    ),
    # These simulated devices need to provide an offset as well for the Nexus file
    # (because they are used by sgl and sgu alias devices, which can also point to real
    # motors with offsets).
    gl_sim = device('nicos.devices.generic.VirtualMotor',
        description = 'Simulated gl motor',
        abslimits = (-15, 15),
        speed = 0,
        unit = 'degree',
    ),
    gu_sim = device('nicos.devices.generic.VirtualMotor',
    ...
    omm = device('nicos_sinq.devices.epics.sinqmotor_deprecated.SinqMotor',
        description = 'Monochromator rotation',
        motorpv = pvmcu3 + 'omm',
    ),
    tlm = device('nicos_sinq.devices.epics.sinqmotor_deprecated.SinqMotor',
        description = 'Monochromator lower translation',
        motorpv = pvmcu3 + 'tlm',
    ),
    tum = device('nicos_sinq.devices.epics.sinqmotor_deprecated.SinqMotor',
    ...
    vsr = device('nicos_sinq.devices.epics.sinqmotor_deprecated.SinqMotor',
        description = 'Virtual slit right',
        motorpv = pvmcu4 + 'vsr',
    ),
    mono = device('nicos_sinq.camea.devices.cameamono.CameaMono',
        ...
        unit = 'meV',
        focmode = 'vertical',
        vfocuspars = [21.027, 91.619],
        abslimits = [2.75, 17],
        focusv = 'mcv',
        focush = 'mch',
        hfocuspars = [10.566, 281.933],
        upper_trans = [0.0, 0.0],
        lower_trans = [0.0, 0.0],
        lower = 'tlm',
        upper = 'tum',
