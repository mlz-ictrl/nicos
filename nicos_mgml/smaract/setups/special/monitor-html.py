...
_boosterblock = Block('Booster magnet', [
    BlockRow(Field(name='B', dev='B_booster'),
             Field(key='B_booster/target', name='Target', unit='T'),
             Field(key='B_booster/ramp', name='Ramp', unit='T/min'),
             ),
    BlockRow(
             Field(key='B_booster/status[1]', name='status'),
             ),
    ],
    setups='magnet',
)
...
_tempblock = Block('Temperature', [
    BlockRow(Field(dev='T'), Field(key='t/setpoint', name='Setpoint'),
                   Field(dev='Ts'), Field(key='ts/setpoint', name='Setpoint')),
    BlockRow(Field(dev='T', plot='T', plotwindow=3600*3, width=50, height=50),
            Field(dev='Ts', name='Ts', plot='T', plotwindow=3600*3))
    ],
    setups='ls',
)
...
_colA = Column(_magnetblock, _boosterblock)
...
devices = dict(
    Monitor = device('nicos.services.monitor.html.Monitor',
        title = 'Smaract status monitor',
        filename = '/mnt/statmons/smaract.html',
        loglevel = 'info',
        interval = 5,
