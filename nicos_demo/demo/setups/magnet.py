...
devices = dict(
    ...
    garfield_symmetry = device('nicos.devices.generic.ManualSwitch',
        description = 'symmetry switch',
        states = ['symmetric', 'short', 'unsymmetric'],
    ),
    garfield_current = device('nicos.devices.generic.VirtualMotor',
        description = 'current source for garfield test',
        abslimits = (0, 250),
        unit = 'A',
        ramp = 1.,
    ),
    B_garfield = device('nicos_mlz.devices.amagnet.GarfieldMagnet',
        ...
        calibrationtable = dict(
            symmetric = (0.00186517, 0.0431937, -0.185956, 0.0599757, 0.194042),
            unsymmetric = (0.00136154, 0.027454, -0.120951, 0.0495289,
                           0.110689),
        ),
    ),
