...
class DevInfo(AttrDict):
    ...
    def fmtParam(self, param, value, unit=True):
        ...
        if info:
            ...
            if not unit:
                return fmtvalue
            return fmtvalue + ' ' + (info['unit'] or '').replace('main',
                                                                 self.unit)
        return str(value)
...
class ControlDialog(QDialog):
        ...
        for key, value in sorted(params.items()):
            if self.devinfo.params.get(key):
                ...
                if is_userparam or self.device_panel._show_lowlevel:
                    punit = (self.devinfo.params[key]['unit'] or ''
                             ).replace('main', mainunit)
                    self.paramItems[key] = item = QTreeWidgetItem(
                        self.paramList,
                        [key, punit, self.devinfo.fmtParam(key, value, False)])
    ...
    def on_cache(self, subkey, value):
        ...
        self.paramItems[subkey].setText(
            2, self.devinfo.fmtParam(subkey, value, False))
