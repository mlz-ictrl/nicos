...
#
# Description:
#   NICOS daemon thread for executing user scripts
#
# Author:
...
from nicm.loggers import INPUT
from nicm.daemon.util import format_exception_cut_frames, format_script, \
     format_timestamp, fixup_script, update_linecache
from nicm.daemon.pyctl import Controller, ControlStop
...
class ExecutionController(Controller):
    ...
    def new_request(self, request):
        ...
        if self.in_startup:
            raise RequestError('nicm setup not finished')
    ...
    def script_thread_entry(self):
        ...
        try:
            ...
            setup_code = 'from nicm import nicos; nicos.loadSetup(%r)' % \
                         self.setup
            # this is to allow the traceback module to report the script's
            # source code correctly
            ...
            try:
                ...
                nicos.logUnhandledException(msg='Error loading nicm:')
