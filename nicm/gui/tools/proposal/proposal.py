...
from nicm.gui import custom
from nicm.gui.tools.uitools import DlgPresets, runDlgStandalone, selectOutputFile
...
''' % (samplename, samplename,
    ...
    def _archiveDataFiles(self, pdlg):
        ...
        for i in days:
            ...
            if custom.NICMLOGPATH:
                for fname in [
                    path.join(custom.NICMLOGPATH, 'cmd%s.log' % fmt1),
                    path.join(custom.NICMLOGPATH, 'error%s.log' % fmt1)
                    ]:
