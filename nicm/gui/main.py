...
from nicm.gui.data import DataHandler, DataError
from nicm.gui.util import DlgUtils, SettingGroup, loadUi, dialogFromUi, chunks, \
     get_display, parse_conndata, enumerateWithProgress, setForegroundColor, \
     setBackgroundColor
from nicm.gui.client import NicosClient, STATUS_INBREAK, STATUS_IDLE
from nicm.gui.editor import EditorWindow
from nicm.gui.toolsupport import main_tools, HasTools

from nicm.gui.custom import has_customization, list_customizations

try:
    # needs Qwt5, which may not be available, so make it optional
    from nicm.gui.analysis import AnalysisWindow
...
class MainWindow(QMainWindow, HasTools, DlgUtils):
    ...
    def on_actionAbout_triggered(self):
        QMessageBox.information(
            ...
            self.tr('NICM GUI client version %1, '
                    'written by Georg Brandl.\n\nServer: ').arg(nicm_version)
    ...
    def on_actionReload_triggered(self):
        if self.client.send_command('reload_nicm', True):
            QMessageBox.information(self, self.tr('Reload'),
                                    self.tr('NICM system reloaded on server.'))
    ...
    def set_status(self, status):
        ...
        # title bar change
        self.setWindowTitle((str(self.instrument) or 'NICM') + ' - ' +
...
def main(argv):
    ...
    import nicm.gui.gui_rc

    app = QApplication(argv)
    app.setOrganizationName('frm2')
    app.setApplicationName('nicmv2')
