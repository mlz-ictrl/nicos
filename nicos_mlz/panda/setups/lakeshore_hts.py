...
devices = dict(
    T_ls_hts = device('nicos.devices.entangle.TemperatureController',
    ...
    T_ls_hts_D = device('nicos.devices.entangle.Sensor',
        description = 'Thermoelement Sensor D',
        tangodevice = tango_base + 'ls_hts/sensord',
        maxage = 6,
    ),
    ls_hts_heaterpower = device('nicos.devices.entangle.AnalogOutput',
