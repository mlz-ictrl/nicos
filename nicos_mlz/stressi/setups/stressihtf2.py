...
devices = {
    ...
    'n2': device('nicos.devices.taco.NamedDigitalOutput',
        description = 'N2 valve control',
        tacodevice = '//%s/box/plc/gas1' % nethost,
        mapping = {'closed': 0,
                   'open': 1}
    ),
    'he': device('nicos.devices.taco.NamedDigitalOutput',
        description = 'He valve control',
        tacodevice = '//%s/box/plc/gas2' % nethost,
        mapping = {'closed': 0,
                   'open': 1}
    ),
    'lamps': device('nicos.devices.taco.NamedDigitalOutput',
        description = 'Halogen lamps control',
        tacodevice = '//%s/box/plc/onoff' % nethost,
        mapping = {'off': 0,
                   'on': 1}
    ),
    'water': device('nicos.devices.taco.NamedDigitalInput',
