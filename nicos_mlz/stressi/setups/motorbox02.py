description = 'sample table devices'

group = 'plugplay'

includes = ['aliases']

tangobase = 'tango://motorbox02.stressi.frm2.tum.de:10000/box/'

devices = dict(
    tths_m = device('nicos.devices.tango.Motor',
        description = 'TTHS motor',
        tangodevice = tangobase + 'channel1/motor',
        lowlevel = True,
    ),
    tths_c = device('nicos.devices.tango.Sensor',
        description = 'TTHS coder',
        tangodevice = tangobase + 'channel1/coder',
        lowlevel = True,
    ),
    tths_s = device('nicos.devices.generic.Axis',
        description = 'TTHS',
        motor = 'tths_m',
        coder = 'tths_c',
        precision = 0.01,
    ),
    omgs_m = device('nicos.devices.tango.Motor',
        description = 'OMGS motor',
        tangodevice = tangobase + 'channel2/motor',
        lowlevel = True,
    ),
    omgs_c = device('nicos.devices.tango.Sensor',
        description = 'OMGS coder',
        tangodevice = tangobase + 'channel2/coder',
        lowlevel = True,
    ),
    omgs_s = device('nicos.devices.generic.Axis',
        description = 'OMGS',
        motor = 'omgs_m',
        coder = 'omgs_c',
        precision = 0.01,
    ),
    xt_m = device('nicos.devices.tango.Motor',
        description = 'XT motor',
        tangodevice = tangobase + 'channel5/motor',
        lowlevel = True,
    ),
    xt_c = device('nicos.devices.tango.Sensor',
        description = 'XT coder',
        tangodevice = tangobase + 'channel5/coder',
        lowlevel = True,
    ),
    xt_s = device('nicos.devices.generic.Axis',
        description = 'XT',
        motor = 'xt_m',
        coder = 'xt_c',
        precision = 0.01,
    ),
    yt_m = device('nicos.devices.tango.Motor',
        description = 'YT motor',
        tangodevice = tangobase + 'channel6/motor',
        lowlevel = True,
    ),
    yt_c = device('nicos.devices.tango.Sensor',
        description = 'YT coder',
        tangodevice = tangobase + 'channel6/coder',
        lowlevel = True,
    ),
    yt_s = device('nicos.devices.generic.Axis',
        description = 'YT',
        motor = 'yt_m',
        coder = 'yt_c',
        precision = 0.01,
    ),
    zt_m = device('nicos.devices.tango.Motor',
        description = 'ZT motor',
        tangodevice = tangobase + 'channel7/motor',
        lowlevel = True,
    ),
    zt_c = device('nicos.devices.tango.Sensor',
        description = 'ZT coder',
        tangodevice = tangobase + 'channel7/coder',
        lowlevel = True,
    ),
    zt_s = device('nicos.devices.generic.Axis',
        description = 'ZT',
        motor = 'zt_m',
        coder = 'zt_c',
        precision = 0.01,
    ),
)

alias_config = {
    'tths':  {'tths_s': 200,},
    'omgs': {'omgs_s': 200,},
    'xt': {'xt_s': 200,},
    'yt': {'yt_s': 200,},
    'zt': {'zt_s': 200,},
}
