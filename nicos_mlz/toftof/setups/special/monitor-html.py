...
_reactorblock = Block('Reactor', [
    BlockRow(
        Field(name='Power', dev='ReactorPower'),
    ),
    ],
)
...
_chopperblock = Block('Chopper system', [
    BlockRow(
        Field(name='Wavelength', dev='chWL', format='%.1f'),
    ),
    BlockRow(
    ...
    # BlockRow(
    #     Field(name='Ratio', dev='chRatio'),
    #     Field(name='CRC', dev='chCRC'),
    #     Field(name='Slit type', dev='chST'),
    # ),
    # '---',
    # BlockRow(
    #     Field(name='Disk 1', dev='chDS[0]', format='%.1f'),
    ...
    # '---',
    # BlockRow(
    #     Field(name='Flow in (FAK40)', dev='flow_in_ch_cooling'),
    # ),
    # '---',
    # BlockRow(
    #     Field(name='Gauge 1', dev='vac0', format='%.2g'),
...
_choppercoolingblock = Block('Cooling system (Chopper)', [
    BlockRow(
        Field(name='T in', dev='t_in_ch_cooling'),
...
_collimationblock = Block('Radial Collimator/Collimation', [
    BlockRow(
        Field(name='Radial', dev='rc_motor'),
...
_samplecoolingblock = Block('Cooling system (Sample)', [
    ...
    setups='julabo01 or ccmhts01 or ccr* or htf* or ccm*',
)

_ccrblock = Block('Sample environment', [
    BlockRow(
        Field(name='Stick', dev='T_ccr17_stick'),
...
_julabo01block = Block('Sample environment', [
    BlockRow(
        Field(name='Julabo', dev='T_julabo01'),
    ),
    ],
    setups='julabo01',
)
...
_temperatureblock = Block('Sample environment', [
    BlockRow(
        Field(name='Sample Temperature', dev='T'),
    ),
    ],
...
_magnetblock = Block('Sample environment', [
    BlockRow(
        Field(name='Magnetic field', dev='B'),
    ),
    ],
    setups='ccm*',
)
...
_measblock = Block('Measurement', [
    BlockRow(
        Field(key='det/timechannels', name='Time channels'),
    ),
    BlockRow(
        Field(name='Time', dev='timer', format='%.1f'),
...
_rateblock = Block('Detector rates', [
    # BlockRow(
    #     Field(gui='nicos_mlz/toftof/gui/ratespanel.ui'),
    # ),
    ],
    setups='toftof',
)
