...
devices = dict(
    ...
    #     type = 'monitor',
    # ),
    # qm_ctr3 = device('nicos.devices.vendor.qmesydaq.tango.CounterChannel',
    #     description = 'QMesyDAQ Counter3',
    #     tangodevice = tango_base + 'counter3',
    #     type = 'monitor',
    # ),
    # qm_ev = device('nicos.devices.vendor.qmesydaq.tango.CounterChannel',
    ...
    image = device('nicos_mlz.refsans.devices.detector.ImageChannel',
        description = 'QMesyDAQ Image',
        tangodevice = tango_base + 'image',
    ),
    det_pixsize = device('nicos.devices.generic.ReadonlyParamDevice',
        description = 'Pixel size of detector',
        parameter = 'pixel_size',
        device = 'image',
    ),
    det_type = device('nicos.devices.generic.ReadonlyParamDevice',
