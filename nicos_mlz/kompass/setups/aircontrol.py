...
devices = dict(
    spare_motor_x2 = device('nicos.devices.tango.Motor',
    ...
    key = device('nicos.devices.tango.NamedDigitalOutput',
...
for key in ('analyser', 'detector', 'sampletable'):
    devices['airpad_' + key] = device('nicos.devices.tango.NamedDigitalOutput',
        description = 'switches the airpads at %s' % key,
        tangodevice = tango_base + 'airpads_%s' % key,
        mapping = dict(on=1, off=0),
    )
    devices['p_%s' % key] = device('nicos.devices.tango.Sensor',
...
for key in ('ana', 'arm', 'det', 'st'):
    for idx in (1, 2, 3):
        devices['p_airpad_%s_%d' % (key, idx)] = device('nicos.devices.tango.Sensor',
...
for key in (1, 2, 3, 4):
    devices['aircontrol_t%d' % key] = device('nicos.devices.tango.Sensor',
...
#    devices['msb%d' % key] = device('nicos.devices.tango.NamedDigitalOutput',
