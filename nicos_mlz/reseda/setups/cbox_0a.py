...
devices = dict(
    cbox_0a_freq = device('nicos.devices.tango.AnalogOutput',
        description = 'Frequency generator amplitude',
        tangodevice = '%s/cbox_0a/fg_frequency' % tango_base,
        pollinterval = 3,
        fmtstr = '%.3g'
    ),
    cbox_0a_amp = device('nicos.devices.tango.AnalogOutput',
        description = 'Frequency generator amplitude',
        tangodevice = '%s/cbox_0a/fg_amplitude' % tango_base,
        pollinterval = 3,
    ),
    cbox_0a_fwdp = device('nicos.devices.tango.AnalogInput',
        description = 'Power amplifier forward power',
        tangodevice = '%s/cbox_0a/pa_fwdp' % tango_base,
        pollinterval = 3,
    ),
    cbox_0a_revp = device('nicos.devices.tango.AnalogInput',
        description = 'Power amplifier reverse power',
        tangodevice = '%s/cbox_0a/pa_revp' % tango_base,
        pollinterval = 3,
    ),
    cbox_0a_gain = device('nicos.devices.tango.AnalogOutput',
        description = 'Power amplifier gain',
        tangodevice = '%s/cbox_0a/pa_gain' % tango_base,
        pollinterval = 3,
    ),
    cbox_0a = device('nicos_mlz.reseda.devices.cbox.CBoxResonanceFrequency',
        pollinterval = 3,
        description = 'CBox',
        unit = 'Hz',
        power_divider = device('nicos.devices.tango.DigitalOutput',
            description = 'Power divider to split the power for both coils',
            tangodevice = '%s/cbox_0a/plc_power_divider' % tango_base,
        ...
        highpass = device('nicos.devices.tango.DigitalOutput',
            ...
            tangodevice = '%s/cbox_0a/plc_highpass' % tango_base,
            lowlevel = False,  # temporary due to inaccurate auto tune
        ),
        ...
        coil2_transformer = device('nicos.devices.tango.DigitalOutput',
            ...
            tangodevice = '%s/cbox_0a/plc_b_transformer' % tango_base,
            lowlevel = False,  # temporary due to inaccurate auto tune
        ),
    ),
)
