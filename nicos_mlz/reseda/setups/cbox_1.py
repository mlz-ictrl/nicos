...
devices = dict(
    cbox_1_freq = device('nicos.devices.tango.AnalogOutput',
        description = 'Frequency generator amplitude',
        tangodevice = '%s/cbox_1/fg_frequency' % tango_base,
        pollinterval = 3,
        fmtstr = '%.3g'
    ),
    cbox_1_amp = device('nicos.devices.tango.AnalogOutput',
        description = 'Frequency generator amplitude',
        tangodevice = '%s/cbox_1/fg_amplitude' % tango_base,
        pollinterval = 3,
    ),
    cbox_1_fwdp = device('nicos.devices.tango.AnalogInput',
        description = 'Power amplifier forward power',
        tangodevice = '%s/cbox_1/pa_fwdp' % tango_base,
        pollinterval = 3,
    ),
    cbox_1_revp = device('nicos.devices.tango.AnalogInput',
        description = 'Power amplifier reverse power',
        tangodevice = '%s/cbox_1/pa_revp' % tango_base,
        pollinterval = 3,
    ),
    cbox_1_gain = device('nicos.devices.tango.AnalogOutput',
        description = 'Power amplifier gain',
        tangodevice = '%s/cbox_1/pa_gain' % tango_base,
        pollinterval = 3,
    ),
    cbox_1 = device('nicos_mlz.reseda.devices.cbox.CBoxResonanceFrequency',
        pollinterval = 3,
        description = 'CBox',
        unit = 'Hz',
        power_divider = device('nicos.devices.tango.DigitalOutput',
            description = 'Power divider to split the power for both coils',
            tangodevice = '%s/cbox_1/plc_power_divider' % tango_base,
        ...
        highpass = device('nicos.devices.tango.DigitalOutput',
            ...
            tangodevice = '%s/cbox_1/plc_highpass' % tango_base,
            lowlevel = False,  # temporary due to inaccurate auto tune
        ),
        pa_fwdp = 'cbox_1_fwdp',
        pa_revp = 'cbox_1_revp',
        ...
        coil2_transformer = device('nicos.devices.tango.DigitalOutput',
            ...
            tangodevice = '%s/cbox_1/plc_b_transformer' % tango_base,
            lowlevel = False,  # temporary due to inaccurate auto tune
        ),
    ),
)
