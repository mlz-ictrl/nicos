description = 'sample table'
group = 'lowlevel'

includes = ['alias_sth']
excludes = ['sample_ext']

tango_base = 'tango://miractrl.mira.frm2:10000/mira/'

devices = dict(
    co_stt = device('nicos.devices.entangle.Sensor',
        lowlevel = True,
        tangodevice = tango_base + 'sample/phi_enc',
        unit = 'deg',
    ),
    mo_stt = device('nicos.devices.entangle.Motor',
        lowlevel = True,
        tangodevice = tango_base + 'sample/phi_mot',
        unit = 'deg',
        precision = 0.002,
    ),
    stt = device('nicos_mlz.mira.devices.axis.HoveringAxis',
        description = 'sample two-theta angle',
        abslimits = (-5, 120),
        motor = 'mo_stt',
        coder = 'co_stt',
        startdelay = 1,
        stopdelay = 2,
        switch = 'air_sample',
        switchvalues = (0, 1),
        fmtstr = '%.3f',
        precision = 0.002,
    ),
    air_mono = device('nicos.devices.entangle.DigitalOutput',
        tangodevice = tango_base + 'air/mono',
        lowlevel = True,
    ),
    air_sample = device('nicos_mlz.mira.devices.refcountio.RefcountDigitalOutput',
        tangodevice = tango_base + 'air/sample',
        lowlevel = True,
    ),
    air_ana = device('nicos_mlz.mira.devices.refcountio.RefcountDigitalOutput',
        tangodevice = tango_base + 'air/det',
        lowlevel = True,
    ),
    co_sth_st = device('nicos.devices.entangle.Sensor',
        lowlevel = True,
        tangodevice = tango_base + 'sample/om_enc',
        unit = 'deg',
    ),
    mo_sth_st = device('nicos.devices.entangle.Motor',
        lowlevel = True,
        tangodevice = tango_base + 'sample/om_mot',
        unit = 'deg',
        precision = 0.001,
    ),
    sth_st = device('nicos.devices.generic.Axis',
        description = 'sample theta angle',
        motor = 'mo_sth_st',
        coder = 'co_sth_st',
        fmtstr = '%.3f',
        precision = 0.001,
    ),
    co_stz = device('nicos.devices.entangle.Sensor',
        lowlevel = True,
        tangodevice = tango_base + 'sample/stz_enc',
        unit = 'mm',
    ),
    mo_stz = device('nicos.devices.entangle.Motor',
        lowlevel = True,
        tangodevice = tango_base + 'sample/stz_mot',
        unit = 'mm',
        precision = 0.1,
    ),
    stz = device('nicos.devices.generic.Axis',
        description = 'sample translation in vertical direction',
        motor = 'mo_stz',
        coder = 'co_stz',
        fmtstr = '%.2f',
        precision = 0.1,
    ),
    co_stx = device('nicos.devices.entangle.Sensor',
        lowlevel = True,
        tangodevice = tango_base + 'sample/stx_enc',
        unit = 'mm',
    ),
    mo_stx = device('nicos.devices.entangle.Motor',
        lowlevel = True,
        tangodevice = tango_base + 'sample/stx_mot',
        unit = 'mm',
        precision = 0.01,
    ),
    stx = device('nicos.devices.generic.Axis',
        description = 'sample translation along the beam (for sth_st=0)',
        motor = 'mo_stx',
        coder = 'co_stx',
        fmtstr = '%.2f',
        precision = 0.01,
    ),
    co_sty = device('nicos.devices.entangle.Sensor',
        lowlevel = True,
        tangodevice = tango_base + 'sample/sty_enc',
        unit = 'mm',
    ),
    mo_sty = device('nicos.devices.entangle.Motor',
        lowlevel = True,
        tangodevice = tango_base + 'sample/sty_mot',
        unit = 'mm',
        precision = 0.01,
    ),
    sty = device('nicos.devices.generic.Axis',
        description = 'sample translation horizontally perpendicular to the beam (for sth_st=0)',
        motor = 'mo_sty',
        coder = 'co_sty',
        fmtstr = '%.2f',
        precision = 0.01,
    ),
    co_sgx = device('nicos.devices.entangle.Sensor',
        lowlevel = True,
        tangodevice = tango_base + 'sample/sgx_enc',
        unit = 'deg',
    ),
    mo_sgx = device('nicos.devices.entangle.Motor',
        lowlevel = True,
        tangodevice = tango_base + 'sample/sgx_mot',
        unit = 'deg',
        precision = 0.01,
    ),
    sgx = device('nicos.devices.generic.Axis',
        description = 'sample tilt around beam axis (for sth_st=0)',
        motor = 'mo_sgx',
        coder = 'co_sgx',
        fmtstr = '%.2f',
        precision = 0.01,
    ),
    co_sgy = device('nicos.devices.entangle.Sensor',
        lowlevel = True,
        tangodevice = tango_base + 'sample/sgy_enc',
        unit = 'deg',
    ),
    mo_sgy = device('nicos.devices.entangle.Motor',
        lowlevel = True,
        tangodevice = tango_base + 'sample/sgy_mot',
        unit = 'deg',
        precision = 0.01,
    ),
    sgy = device('nicos.devices.generic.Axis',
        description = 'sample tilt orthogonal to beam axis (for sth_st=0)',
        motor = 'mo_sgy',
        coder = 'co_sgy',
        fmtstr = '%.2f',
        precision = 0.01,
    ),
    vg1 = device('nicos.devices.tas.VirtualGonio',
        description = 'Gonio along orient1 reflex',
        cell = 'Sample',
        gx = 'sgx',
        gy = 'sgy',
        axis = 1,
        unit = 'deg',
    ),
    vg2 = device('nicos.devices.tas.VirtualGonio',
        description = 'Gonio along orient2 reflex',
        cell = 'Sample',
        gx = 'sgx',
        gy = 'sgy',
        axis = 2,
        unit = 'deg',
    ),
)

alias_config = {
    'sth': {'sth_st': 0},
}
