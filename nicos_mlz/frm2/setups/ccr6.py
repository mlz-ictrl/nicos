...
devices = {
    ...
    'T_%s_340' % setupname : device('nicos.devices.entangle.TemperatureController',
    ...
    'T_%s_340_range' % setupname : device('nicos.devices.entangle.NamedDigitalOutput',
        description = 'Heaterrange of the control loop of the LS-340',
        tangodevice = tango_base + 'ls2/range1',
        mapping = dict(off=0, low=1, medium=2, high=3),
    ),

    'T_%s_A_340' % setupname : device('nicos.devices.entangle.Sensor',
        description = 'CCR6 sensor A (LS-340)',
        tangodevice = tango_base + 'ls2/sensora',
        unit = 'K',
        fmtstr = '%.3f',
    ),
    'T_%s_B_340' % setupname : device('nicos.devices.entangle.Sensor',
