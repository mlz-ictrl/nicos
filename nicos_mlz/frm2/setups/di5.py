...
devices = {
    ...
    f'{setupname}_R_mc':
        device('nicos.devices.entangle.Sensor',
               ...
               fmtstr = '%.3f', pollinterval = 10, unit = 'Ohm',
               visibility = (),),
    f'{setupname}_compressor':
        device('nicos.devices.entangle.NamedDigitalInput',
               description = 'Compressor',
               tangodevice = tango_base + f'triton/compressor',
               mapping = {'off': 0, 'on': 1,},),
    f'{setupname}_forepump':
        device('nicos.devices.entangle.NamedDigitalInput',
               description = 'Forepump',
               tangodevice = tango_base + f'triton/forepump',
               mapping = {'off': 0, 'on': 1,},),
}
...
for i in [1, 2, 3, 5]:
    devices[f'{setupname}_p{i}'] = \
        device('nicos.devices.entangle.Sensor',
               description = {1: 'Tank pressure',
                              2: 'Condense pressure',
                              3: 'Still pressure',
                              5: 'Compressor pressure',}[i],
               tangodevice = tango_base + f'triton/p{i}',
               fmtstr = '%.3g', pollinterval = 1,)

for i in [1, 2, 4, 5, 9]:
    devices[f'{setupname}_v{i}'] = \
        device('nicos.devices.entangle.NamedDigitalInput',
               description = f'Valve {i}',
               tangodevice = tango_base + f'triton/v{i}',
               fmtstr = '%.3g', pollinterval = 1,
