...
devices = {
    ...
    # coldhead
    f'{setupname}_coldhead': device('nicos_mlz.devices.f70_coldhead.F70ColdheadController',
        description = 'Compressor for Coldhead (should be ON)',
        tangodevice = plc_tango_base + 'coldhead',
        mapping = {'on': 1, 'off': 0},
        ophours_device = plc_tango_base + 'coldhead_ophours',
        p_return_device = plc_tango_base + 'coldhead_p1',
        t_compr_device = plc_tango_base + 'coldhead_t1',
        t_water_in_device = plc_tango_base + 'coldhead_t2',
        t_water_out_device = plc_tango_base + 'coldhead_t3',
    ),
    # sample tube valves
    ...
    f'{setupname}_vacuum_switch': device('nicos.devices.entangle.NamedDigitalOutput',
        ...
        tangodevice = plc_tango_base + 'v_vacuum',
        mapping = {'open': 1, 'closed': 0},
    ),
    # pressure sensors
