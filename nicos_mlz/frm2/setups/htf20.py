...
devices = {
    ...
    '%s_heater' % setupname: device('nicos.devices.entangle.NamedDigitalOutput',
        description = 'On/Off-State of the heater',
        tangodevice = tango_base + 'plc/_heater',
        mapping = {'on': 1, 'off': 0}
    ),
    '%s_state' % setupname: device('nicos.devices.entangle.NamedDigitalInput',
    ...
    '%s_turbo' % setupname: device('nicos.devices.entangle.NamedDigitalInput',
        description = 'State of the turbo pump',
        tangodevice = tango_base + 'plc/_turbo',
        mapping = {'on' : 1, 'off': 0}
    ),
    '%s_exgas_mode' % setupname: device('nicos.devices.entangle.NamedDigitalOutput',
        description = 'Exgas_mode',
        tangodevice = tango_base + 'plc/_exgas_mode',
        mapping = {'on': 1, 'off': 0}
    ),
    '%s_vac_valve' % setupname: device('nicos.devices.entangle.NamedDigitalOutput',
